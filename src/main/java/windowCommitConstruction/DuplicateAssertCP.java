...
import windowCommitConstruction.testSmellPanel.DASmellPanel;
...
public class DuplicateAssertCP extends JPanel implements ListSelectionListener {

    private ArrayList<DuplicateAssertInfo> classesWithDuplicateAssert;
    ...
    public DuplicateAssertCP(ArrayList<DuplicateAssertInfo> classesWithDA, Project project){
    ...
    protected void updateSmellPanel (int index) {
        DuplicateAssertInfo da;
        if(index == -1 && model.getSize() == 0){
            return;
        } else if(index == -1 && model.getSize() != 0){
            da = (DuplicateAssertInfo) classList.getModel().getElementAt(0);
        } else {
            da = (DuplicateAssertInfo) classList.getModel().getElementAt(index);
        }

        secondSplitPane = new DASmellPanel( da, project, this);
    ...
    public void doAfterRefactor(){
        ...
        classesWithDuplicateAssert.remove(index);
        model.remove(index);
