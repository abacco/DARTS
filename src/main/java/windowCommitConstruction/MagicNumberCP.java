...
public class MagicNumberCP extends JPanel implements ListSelectionListener {

    private ArrayList<MagicNumberTestInfo> classesWithMagicNumber;
    ...
    public MagicNumberCP(ArrayList<MagicNumberTestInfo> classesWithMN, Project project){
        ...
        if(!classesWithMN.isEmpty()){
            ...
            secondSplitPane = new MNSmellPanel(classesWithMagicNumber.get(0), project, this);
    ...
    protected void updateSmellPanel (int index) {
        MagicNumberTestInfo mni;
        if(index == -1 && model.getSize() == 0){
            return;
        } else if(index == -1 && model.getSize() != 0){
            mni = (MagicNumberTestInfo) classList.getModel().getElementAt(0);
        } else {
            mni = (MagicNumberTestInfo) classList.getModel().getElementAt(index);
        }
        secondSplitPane = new MNSmellPanel( mni, project, this);
    ...
    public void doAfterRefactor(){
        ...
        classesWithMagicNumber.remove(index);
        model.remove(index);
