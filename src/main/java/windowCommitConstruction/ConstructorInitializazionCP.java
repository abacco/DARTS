...
public class ConstructorInitializazionCP extends JPanel implements ListSelectionListener {

    private ArrayList<ConstructorInitializationInfo> classesWithConstructorInitialization;
    ...
    public ConstructorInitializazionCP(ArrayList<ConstructorInitializationInfo> classesWithCI, Project project){
        ...
        if(!classesWithCI.isEmpty()) {
            //Parte relativa all'inizializzazione del Panel per GeneralFixture.
            classesWithConstructorInitialization = classesWithCI;

            for(ConstructorInitializationInfo cii : classesWithConstructorInitialization){
                model.addElement(cii);
            }
            ...
            secondSplitPane = new CIISmellPanel(classesWithConstructorInitialization.get(0), project, this);
    ...
    protected void updateSmellPanel (int index) {
        ConstructorInitializationInfo cii;
        if(index == -1 && model.getSize() == 0){
            return;
        } else if(index == -1 && model.getSize() != 0){
            cii = (ConstructorInitializationInfo) classList.getModel().getElementAt(0);
        } else {
            cii = (ConstructorInitializationInfo) classList.getModel().getElementAt(index);
        }
        secondSplitPane = new CIISmellPanel( cii, project, this);
    ...
    public void doAfterRefactor(){
        ...
        classesWithConstructorInitialization.remove(index);
        model.remove(index);
